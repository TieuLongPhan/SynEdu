...
This talktorial (**S03**) introduces **maximum common substructure (MCS)** as a practical alignment tool for reaction informatics when exact graph isomorphism is too strict.

We focus on two connected ideas:

1. **MCS-based molecular alignment**  
   Using the largest shared subgraph to compare molecules that differ by edits, substitutions, or missing components.

2. **Reaction rebalancing**  
   Combining MCS alignment with rule-based reasoning to detect missing species and make reaction records more consistent.

We use:

- **RDKit** for chemistry-aware MCS behavior, including atom/bond comparison settings, aromaticity, and ring constraints <a href="#ref-1">[1]</a>.
- **NetworkX** for explicit graph-level alignment logic and symmetry-aware interpretation <a href="#ref-2">[2]</a>.

The goal is to connect molecular similarity with reaction reasoning: an MCS is not only a similarity score, but also a candidate correspondence that can support atom mapping, reaction-center localization, and data curation.
...
The **Maximum Common Substructure (MCS)** [3][4] problem seeks the largest subgraph
...
**Remark.** MCS computation is NP-hard in general. RDKit uses an FMCS heuristic that may return a *locally* maximal but not globally maximal common subgraph. NetworkX-based MCS uses exact backtracking, which is exponential in the worst case but exact.
...
Tanimoto similarity (fingerprint-based) and MCS size (graph-based) both measure molecular overlap, but they are **not equivalent**. The scatter plot below samples molecules from the dataset and compares both metrics against ethanol. Ring-containing molecules can have high graph MCS under RDKit's chemistry-aware matcher yet low Tanimoto — or vice versa.


## 2. Rule-based reaction rebalancing

We utilize idea from *[Reaction rebalancing: a novel approach to curating reaction databases](https://link.springer.com/article/10.1186/s13321-024-00875-4)* [6]. 
...
In this section, we directly use functionality from **SynKit** [6]
(`pip install synkit`) and explore an established tool for reaction rebalancing,
**SynRBL** [7] (`pip install synrbl`).
...
Here we demonstrate the same rebalancing task using **SynRBL** [6], a dedicated
